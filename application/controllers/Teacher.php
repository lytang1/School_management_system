...
class Teacher extends CI_Controller
{
	...
	function student_information($class_id = '')
	{
        ...
        $page_data['students']  = $this->db->get_where('student',array('class_id'=>$class_id))->result_array();
		$page_data['function']  = 'student_information';
        $this->load->view('backend/index', $page_data);
	}
        ...
        if($param1 =='create'){
            ...
            foreach ($students as $student) {
                ...
                if($study_time=='Full time'){
                    ...
                    $is_exist1 = $this->db->get_where('attendance',array('student_id'=>$student['student_id'],'date'=>date('d/M/Y',strtotime($this->input->post('att_day')))))->row()->attendance_time_id;
                ...
                $today = date('d/M/Y',strtotime($this->input->post('att_day')));
               $y = date('Y',now());
               $month= date('m',now());
                ...
                if($month>=8){
                    ...
                    $yn = $y +1;
                    $data1['scholar_year'] = $y.'-'.$yn;
                }elseif($month<8){ 
                $yl = $y -1;
                $data1['scholar_year']= $yl.'-'.$y;
            }
    ...
    function attendance_note($param1 ='',$param2 =''){
         ...
         if ($date =='') {
            ...
            $page_data['classes']    = $class->result_array();

            

            $students = $query->result_array();
            $teacher_id = $this->db->get_where('teacher_class',array('class_name_id'=>$class_id))->row()->teacher_id;
    ...
    function view_weekly($param1 ='', $param2 ='',$param3 = ''){
        // Need to do //
        /* get the date of current study year and test if month bigger than Dec or start from 1 then year 
        we get from the year that is smaller and until the bigger should be on july 31 */ 
        
        $date = strtotime('01-Aug-2015');
        $date = date('d/M/Y',$date);

       $attendances = $this->db->get_where('attendance',array('date >'=>$date,'class_id'=>$param1,'student_id'=>$

$y = date('Y',now());
$m = date('m',now());
if($m>=8){$yn = $y +1; $study_year = $y.'-'.$yn;}
if($m<8){$yl = $y -1; $study_year = $yl.'-'.$y;}

        $page_data['student_name'] = $this->db->get_where('student',array('student_id'=>$param2))->row()->name;
        $page_data['class_name']   = $this->db->get_where('class',array('class_id'=>$param1))->row()->name;

        $page_data['study_time'] = $this->db->get_where('class_student',array('class_id'=>$param1,'student_id'=>$param2,'study_year'=>$study_year))->row()->study_time;
        $page_data['attendances']=  $attendances; 
        $page_data['page_name']  = 'view_attendance';
        $page_data['page_title'] = 'Weekly attendance';
        $this->load->view('backend/index',$page_data);       
