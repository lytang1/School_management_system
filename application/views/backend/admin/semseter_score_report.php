    ...
    <a onClick="PrintElem('#print')" class="btn btn-default btn-icon icon-left hidden-print pull-right">
		...
		<table class="table table-bordered"  style="text-align:center" id='both'>
			<tr>
				...
				<th colspan="<?php echo sizeof($subjects);?>"><center>Subjects</center></th>
				<th></th>
			</tr>
			<tr>
				<td><center>Student name</center></td>
				<?php foreach ($subjects as $row_subject):?>
					<td><?php echo $row_subject['name'];?></td>
				<?php endforeach;?>
				<td>Average score</td>
			</tr>
		
			<?php $c=-1; foreach ($students as $row):$c++;$total=0;?>
				
			<tr >
				<td ><?php echo ucfirst($row['name']);?> </td>
				
				<?php for ($j=0;$j<sizeof($average_score);$j++):?>

				<td ><?php echo $average_score[$j][$c]; $total= $total+ $average_score[$j][$c];?>
				</td>
			<?php endfor;?>
				<td><?php $avg = $total/sizeof($average_score); echo $avg; ?></td>
			</tr>
		<?php endforeach;?>
		</table>
