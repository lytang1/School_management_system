...
class Crud_model extends CI_Model {
    ...
    ///----GET STUDENT BORROW BOOK ----///
    function get_student_borrow_book($student_id){
        $this->db->select('*');
        $this->db->from('borrow_list');
    ...
    function get_all_borrow_book(){
        $this->db->select("*,count('student_id') as quantity");
        $this->db->from('borrow_list');
    ...
    //-----GET IMAGE ID OF STUDENT -----//
    function get_image_id($student_id){
    ...
    function get_service_list($invoice_id){
        $query = $this->db->get_where('service_list_to_pay',array('id_invoice'=>$invoice_id));
        return $query->result_array();
    }
    ...
    function get_parent_info($name,$address) {
        $query = $this->db->get_where('parent', array('guardian_name' => $name,'guardian_address'=>$address));
          $res = $query->result_array();
        foreach ($res as $row)
            return $row['parent_id'];
    }

    function get_class_fee($class_id) {
        $query = $this->db->get_where('class', array('class_id' => $class_id));
        $res = $query->result_array();
        foreach ($res as $row)
    ...
    function get_class_enroll_fee($class_id) {
        $query = $this->db->get_where('class', array('class_id' => $class_id));
        $res = $query->result_array();
        foreach ($res as $row)
            return $row['enroll_fee'];
    }
    ...
    function get_classes() {
     ...
     function get_classes_limit($num) {
        $query = $this->db->get('class',$num,20);
        return $query->result_array();
    }
    ...
    ////////IMAGE URL//////////
    function get_image_url($type = '', $id = '') {
        $image_id = $this->db->get_where('student',array('student_id'=>$id))->row()->image_id;

       $image_url = $this->db->get_where('image',array('id'=>$image_id))->row()->image_url;
    ...
    //-----GET DESCRIPTION OF INVOICE------//
    function get_description($student_id){
        $this->db->select('*');
        $this->db->from('service_list_to_pay');
        $this->db->join('invoice','invoice.invoice_id =service_list_to_pay.id_invoice');
        $this->db->where('student_id',$student_id);
